...
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    try {
      const savedFavorites = localStorage.getItem('neon_favorites');
      if (savedFavorites) {
...
function loadSong(index) {
  ...
  const isFav = appState.favorites.includes(song.id);
  const favBtn = document.getElementById('favorite-btn');
  if (favBtn) {
    favBtn.querySelector('i').className = isFav ? 'ri-heart-fill' : 'ri-heart-line';
    favBtn.classList.toggle('active', isFav);
  }
  const epFavBtn = document.getElementById('expanded-favorite-btn');
  if (epFavBtn) {
    epFavBtn.querySelector('i').className = isFav ? 'ri-heart-fill' : 'ri-heart-line';
    epFavBtn.classList.toggle('active', isFav);
  }
...
function nextSong() {
  ...
  const isLastSong = appState.currentSongIndex === appState.playlist.length - 1;

  if (isLastSong && appState.repeatMode === 'none') {
...
function updateVolumeUI() {
  ...
  const epSlider = document.getElementById('ep-volume-slider');
  const volumeBtn = document.getElementById('volume-btn');
  const epVolumeBtn = document.getElementById('ep-volume-btn');

  const volPercent = appState.volume * 100;
  if (slider) slider.value = volPercent;
  if (epSlider) epSlider.value = volPercent;

  // Change volume icon based on level
  const getIconClass = (vol) => {
    if (vol === 0) return 'ri-volume-mute-line';
    else if (vol < 0.5) return 'ri-volume-down-line';
    else return 'ri-volume-up-line';
  };

  const iconClass = getIconClass(appState.volume);
  if (volumeBtn) volumeBtn.querySelector('i').className = iconClass;
  if (epVolumeBtn) epVolumeBtn.querySelector('i').className = iconClass;
}
...
function toggleFavorite(songId) {
  ...
  localStorage.setItem('neon_favorites', JSON.stringify(appState.favorites));
  renderSongs();
  ...
  if (currentSong.id === songId) {
    ...
    if (btn) {
      btn.querySelector('i').className = isFav ? 'ri-heart-fill' : 'ri-heart-line';
      btn.classList.toggle('active', isFav);
    }
    const epBtn = document.getElementById('expanded-favorite-btn');
    if (epBtn) {
      epBtn.querySelector('i').className = isFav ? 'ri-heart-fill' : 'ri-heart-line';
      epBtn.classList.toggle('active', isFav);
    }
  }
}
...
function setupPlayerListeners() {
  ...
  audio.addEventListener('timeupdate', () => {
    ...
    if (epFill) epFill.style.width = `${progressPercent}%`;

    const currentText = formatTime(currentTime);
    const totalText = formatTime(duration || 0);

    const timeCurrent = document.getElementById('time-current');
    const epTimeCurrent = document.getElementById('ep-time-current');
    if (timeCurrent) timeCurrent.textContent = currentText;
    if (epTimeCurrent) epTimeCurrent.textContent = currentText;

    const timeTotal = document.getElementById('time-total');
    const epTimeTotal = document.getElementById('ep-time-total');
    if (timeTotal) timeTotal.textContent = totalText;
    if (epTimeTotal) epTimeTotal.textContent = totalText;
  });
  ...
  const epProgressBar = document.getElementById('ep-progress-bar');
  if (epProgressBar) {
    epProgressBar.addEventListener('click', function (e) {
      const width = this.clientWidth;
      const clickX = e.offsetX;
      audio.currentTime = (clickX / width) * audio.duration;
    });
  ...
  // Expanded Volume Button Click (Mute Toggle)
  const epVolumeBtn = document.getElementById('ep-volume-btn');
  if (epVolumeBtn) {
    epVolumeBtn.addEventListener('click', () => {
      if (audio.volume > 0) {
        appState.savedVolume = appState.volume;
        appState.volume = 0;
      } else {
  ...
  // Expanded Volume Slider Control
  const epVolumeSlider = document.getElementById('ep-volume-slider');
  if (epVolumeSlider) {
    epVolumeSlider.addEventListener('input', (e) => {
      appState.volume = e.target.value / 100;
      audio.volume = appState.volume;
      updateVolumeUI();
      localStorage.setItem('neon_volume', appState.volume);
  ...
  // Action Buttons Listeners (Like, Download, Share, Playlist)
  const favoriteBtn = document.getElementById('favorite-btn');
  const epFavoriteBtn = document.getElementById('expanded-favorite-btn');
  ...
  if (favoriteBtn) favoriteBtn.addEventListener('click', handleFavClick);
  if (epFavoriteBtn) epFavoriteBtn.addEventListener('click', handleFavClick);

  const downloadBtn = document.getElementById('download-btn');
  const epDownloadBtn = document.getElementById('expanded-download-btn');
  if (downloadBtn) downloadBtn.addEventListener('click', downloadTrack);
  if (epDownloadBtn) epDownloadBtn.addEventListener('click', downloadTrack);

  const shareBtn = document.getElementById('share-btn');
  const epShareBtn = document.getElementById('expanded-share-btn');
  if (shareBtn) shareBtn.addEventListener('click', shareTrack);
  if (epShareBtn) epShareBtn.addEventListener('click', shareTrack);

  const playlistBtn = document.getElementById('add-to-playlist-btn');
  const epPlaylistBtn = document.getElementById('expanded-add-playlist-btn');
  if (playlistBtn) playlistBtn.addEventListener('click', handleFavClick);
  if (epPlaylistBtn) epPlaylistBtn.addEventListener('click', handleFavClick);
  ...
  if (queueNextBtn) {
    queueNextBtn.addEventListener('click', () => {
      ...
      const nextIndex = appState.currentSongIndex + 1;
      // Check if it's already the next song
      if (nextIndex < appState.playlist.length && appState.playlist[nextIndex].id.endsWith(currentSong.id)) {
        showNotification('Song is already queued next', 'info');
        return;
      }

      // Insert copy after current song
      const queuedSong = {
        ...currentSong,
        id: `queued-${Date.now()}-${currentSong.id}`
      };
      appState.playlist.splice(nextIndex, 0, queuedSong);
      showNotification('Queued to play next', 'success');
      renderQueue();
      updateNextUpQueue();
    });
  }
  ...
  if (clearQueueBtn) {
    clearQueueBtn.addEventListener('click', () => {
      if (appState.playlist.length > appState.currentSongIndex + 1) {
        ...
        showNotification('No songs in queue to clear', 'info');
      }
    });
  }
...
function handleFolderSelect(event) {
  ...
  Array.from(files).forEach(file => {
    // Robust check for audio MIME types or file extensions (falls back when browser type is empty)
    ...
    if (isAudioType || isAudioExt) {
      ...
      appState.originalPlaylist.push(song); // Keep in original playlist
      addedCount++;
    }
  });
  ...
  if (addedCount > 0) {
    ...
    // Activate the 'Local' filter tab visually
    document.querySelectorAll('.category-tab').forEach(tab => {
      if (tab.dataset.category === 'local') {
        tab.classList.add('active');
      } else {
...
function setupAdvancedListeners() {
  ...
  if (qualitySelect) {
    qualitySelect.addEventListener('change', (e) => {
      ...
      showNotification(`Audio Quality set to ${e.target.value.toUpperCase()}`, 'success');
    });
  }
...
// ==========================================
// 12. UTILITIES & ADVANCED SETTINGS
// ==========================================
...
function loadSettings() {
  try {
    ...
    if (savedVolume !== null) {
      ...
      audio.volume = appState.volume; // Default 0.7
    }
  } catch (e) {
...
/**
 * Initiates file download for the current track
 */
...
/**
 * Shares the current track or copies its details to clipboard
 */
async function shareTrack() {
  ...
  const shareData = {
    title: 'Neon Beats',
    text: `Listen to "${song.title}" by ${song.artist} on Neon Beats!`,
...
document.addEventListener('keydown', (e) => {
  ...
  switch (e.code) {
    ...
    case 'ArrowUp':
      e.preventDefault();
      appState.volume = Math.min(1.0, appState.volume + 0.1);
      audio.volume = appState.volume;
      updateVolumeUI();
      localStorage.setItem('neon_volume', appState.volume);
      break;
    case 'ArrowDown':
      e.preventDefault();
      appState.volume = Math.max(0.0, appState.volume - 0.1);
      audio.volume = appState.volume;
      updateVolumeUI();
      localStorage.setItem('neon_volume', appState.volume);
      break;
